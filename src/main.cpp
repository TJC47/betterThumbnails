...
class $modify(BTNMenuLayer, MenuLayer) {
    bool init() {
        ...
        if (!NotificationOverlay::get()) {
            auto overlay = NotificationOverlay::create();
            overlay->setID("better-thumbnail-notification-overlay");
            if (overlay) {
                OverlayManager::get()->addChild(overlay, 0);
            }
        }
        return true;
    }
