...
but the yellow is annoying sooo)*/
...
bool BetterThumbnailLayer::init() {
  ...
  if (Mod::get()->getSettingValue<bool>("random-thumbnail-background") ==
    ...
    bgImage->loadFromUrl("https://levelthumbs.prevter.me/thumbnail/random",
                         LazySprite::Format::kFmtUnKnown, true);
...
void BetterThumbnailLayer::onMyThumbnail(CCObject *) {
  // to do: my thumbnails
  FLAlertLayer::create("My Thumbnails", "This feature is not implemented yet.",
                       "Ok")
      ->show();
}
void BetterThumbnailLayer::onRecent(CCObject *) {
  ...
  FLAlertLayer::create("Recent Thumbnails",
                       "This feature is not implemented yet.", "Ok")
      ->show();
}
void BetterThumbnailLayer::onPending(CCObject *) {
  if (Mod::get()->getSavedValue<int>("role_num") >= 20) {
    CCDirector::get()->pushScene(
        CCTransitionFade::create(.5f, PendingThumbnailLayer::scene()));
  } else {
...
void BetterThumbnailLayer::onManage(CCObject *) {
  // to do: manage user
  if (Mod::get()->getSavedValue<int>("role_num") >= 30) {
    FLAlertLayer::create("Manage User", "This feature is not implemented yet.",
                         "Ok")
        ->show();
  } else {
    FLAlertLayer::create(
...
void BetterThumbnailLayer::onInfoButton(CCObject *) {
  ...
  auto infoString =
      fmt::format("Rank: {}\nRank (numerical): {}\nUser ID: {}\nActive "
                  ...
                  userRank, userRankNum, userId, activeThumbnails,
                  acceptUploadThumbnailCount, uploadThumbnailCount);
  FLAlertLayer::create(
      Mod::get()->getSavedValue<std::string>("username").c_str(), infoString,
      "Ok")
      ->show();
}