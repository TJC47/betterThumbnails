...
CCScene *ThumbnailInfoLayer::scene(int id, int user_id,
                                   const std::string &username, int level_id,
                                   bool accepted,
                                   const std::string &upload_time,
                                   bool replacement) {
...
ThumbnailInfoLayer *ThumbnailInfoLayer::create(int id, int user_id,
                                               const std::string &username,
                                               int level_id, bool accepted,
                                               const std::string &upload_time,
                                               bool replacement) {
  auto ret = new ThumbnailInfoLayer;
  if (ret && ret->init(id, user_id, username, level_id, accepted, upload_time,
                       replacement)) {
    ret->autorelease();
    return ret;
  }
...
bool ThumbnailInfoLayer::init(int id, int user_id, const std::string &username,
                              int level_id, bool accepted,
                              const std::string &upload_time,
                              bool replacement) {
  if (!CCLayer::init())
  ...
  m_listener.spawn(std::move(imageTask),
                   [this, thumbReplacement, thumbOriginal, thumbBg, id,
                    spinner](web::WebResponse res) {
                     if (res.code() >= 200 && res.code() <= 299) {
                       ...
                       log::error("Image fetch error: {} {}", res.code(),
                                  res.string().unwrapOr(""));
                       if (spinner)
                         spinner->setVisible(false);
                     }
  ...
  if (m_replacementFlag) {
    auto showSpr =
        ButtonSprite::create("Show original", 140, true, "bigFont.fnt",
  ...
  if (Mod::get()->getSavedValue<int>("role_num") >= 20) {
    ...
    auto playBtn = CCMenuItemSpriteExtra::create(
        playBtnSprite, this,
        menu_selector(ThumbnailInfoLayer::onPlayLevelButton));
...
void ThumbnailInfoLayer::onShowOriginal(CCObject *) {
  ...
  if (m_originalLoaded) {
    ...
    if (m_showOriginalBtn) {
      auto newSprite =
          ButtonSprite::create("Show replacement", 160, true, "bigFont.fnt",
                               "GJ_button_01.png", 30.f, 1.f);
      m_showOriginalBtn->setNormalImage(newSprite);
    }
    return;
  ...
  m_listener.spawn(std::move(task), [this](web::WebResponse res) {
    if (res.code() >= 200 && res.code() <= 299) {
      ...
      log::error("Original image fetch error: {} {}", res.code(),
                 res.string().unwrapOr(""));
      if (m_thumbSpinner)
        m_thumbSpinner->setVisible(false);
