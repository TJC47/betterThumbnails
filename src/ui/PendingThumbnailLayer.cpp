...
bool PendingThumbnailLayer::init() {
  ...
  GJListLayer *listLayer = GJListLayer::create(nullptr, "Pending Thumbnails",
                                               {0, 0, 0, 100}, 356.f, 220.f, 0);
  this->addChild(listLayer, 1);
  listLayer->setAnchorPoint(CCPoint(0.5f, 0.5f));
  listLayer->setPosition(
      CCPoint(screenSize / 2 - listLayer->getScaledContentSize() / 2));

  ScrollLayer *scrollLayer =
      ScrollLayer::create(listLayer->getContentSize(), true, true);
  ...
  auto listBottomY =
      screenSize.height / 2.f - listLayer->getContentSize().height / 2.f - 50.f;
  ...
  auto allSpr = ButtonSprite::create("All", widthParam, true, "goldFont.fnt",
                                     "GJ_button_01.png", heightParam, 1.f);
  ...
  auto newSpr = ButtonSprite::create("New", widthParam, true, "goldFont.fnt",
                                     "GJ_button_01.png", heightParam, 1.f);
  ...
  auto repSpr =
      ButtonSprite::create("Replacement", widthParam, true, "goldFont.fnt",
                           "GJ_button_01.png", heightParam, 1.f);
  ...
  auto searchIcon =
      CCSprite::createWithSpriteFrameName("geode.loader/search.png");
  ...
  m_searchFilterBtn->setPosition(
      {listLayer->getPositionX() - 40, listLayer->getContentHeight()});
...
void PendingThumbnailLayer::fetchPage(int page) {
  ...
  m_listener.spawn(std::move(task), [this, url, token](web::WebResponse res) {
    if (res.code() < 200 || res.code() > 299) {
      ...
      log::error("Pending API error: {} {}", res.code(),
                 res.string().unwrapOr(""));
      Notification::create(
    ...
    if (!jsonResult.isOk()) {
      ...
      log::error("Pending API JSON parse error: {}. Body: {}",
                 jsonResult.unwrapErr(), truncated);
      Notification::create(
    ...
    if (json.isObject() && json["uploads"].isArray()) {
      ...
      log::error("Pending API did not return expected uploads object with "
...
void PendingThumbnailLayer::updateUI() {
  if (!m_contentLayer || !m_scrollLayer)
    return;
  m_contentLayer->removeAllChildrenWithCleanup(true);
  ...
  for (int i = start; i < end; ++i) {
    ...
    auto thumbNode = ThumbnailNode::create(
        m_scrollLayer->getContentSize(), item.id, item.user_id, item.username,
        item.level_id, item.accepted, item.upload_time, item.replacement);
    if (thumbNode) {
